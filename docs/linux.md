...
| --- | --- |
| `monitorize/__main__.py` | Application entry point for `python -m monitorize`. |
| `monitorize/desktop/tray_agent.py` | Lightweight login tray that opens the full GUI only when needed. |
| `monitorize/desktop/streaming_controller.py` | Stream, virtual-display, TLS, input, and process lifecycle. |
| `monitorize/platform/kde_virtual_monitor.py` | KDE portal virtual-output detection and safe KScreen mode configuration. |
| `monitorize/streaming/Streamer_kde.py` | KDE ScreenCast portal capture. |
| `monitorize/streaming/portal_streamer.py` | Shared XDG ScreenCast portal session and PipeWire stream handling. |
| `monitorize/streaming/pipeline_builder.py` | CPU, VA-API, or NVENC GStreamer pipeline launch. |
| `monitorize/input_bridge/touch_daemon.py` and `monitorize/input_bridge/` | Android input transport, dispatch, geometry, and uinput handling. |
| `monitorize/desktop/discovery_service.py` | Zeroconf discovery and encrypted/plain capability advertisement. |

## Startup and Tray Behavior

The **Start Monitorize after login** setting writes an XDG autostart entry that launches `python -m monitorize --tray-agent`.
