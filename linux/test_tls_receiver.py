...
class TlsReceiverTest(unittest.TestCase):
    ...
    def test_invalid_local_port_is_rejected(self):
        with (
            patch.object(sys, "argv", ["tls_receiver.py", "host", "7110", "--local-port", "70000"]),
    ...
    def test_fingerprint_mismatch_closes_remote(self):
        ...
        with (
            patch.object(sys, "argv", ["tls_receiver.py", "host", "7110", "--fingerprint", wrong]),
            patch("tls_receiver.create_client_context", return_value=context),
            patch("tls_receiver.socket.create_connection", return_value=object()),
        ):
    ...
    def test_auth_failure_closes_remote(self):
        ...
        token = "a" * 64
        with (
            patch.object(sys, "argv", ["tls_receiver.py", "host", "7110", "--token", token]),
            patch("tls_receiver.create_client_context", return_value=context),
            patch("tls_receiver.socket.create_connection", return_value=object()),
        ):
            self.assertEqual(tls_receiver.main(), 2)
    ...
    def test_listener_bind_failure_closes_remote_and_listener(self):
        ...
        with (
            ...
            patch("tls_receiver.create_client_context", return_value=context),
            patch("tls_receiver.socket.create_connection", return_value=object()),
            patch("tls_receiver.socket.socket", return_value=listener),
