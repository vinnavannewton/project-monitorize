...
def main() -> None:
    ...
    parser.add_argument("--local-port", type=_port_arg, default=17110)
    args = parser.parse_args()

    host = args.host.strip()
    if not host:
        print("[TLS RECEIVER] ERROR invalid host", flush=True)
        return 1

    remote = listener = local = None
    try:
        context = create_client_context()
        remote = context.wrap_socket(
            socket.create_connection((host, args.port), timeout=5),
        ...
        if args.token:
            remote.sendall(f"AUTH {args.token.strip()}\n".encode("ascii"))
        elif args.code:
            remote.sendall(f"PAIR {args.code.strip()}\n".encode("ascii"))
        else:
            print(f"[TLS RECEIVER] AUTH_FAILED {fingerprint}", flush=True)
            return 2
        ...
        if not response.startswith("OK"):
            print(f"[TLS RECEIVER] AUTH_FAILED {fingerprint}", flush=True)
            return 2
        token = args.token.strip() or response.removeprefix("OK ").strip()
        ...
        for sock in (local, listener, remote):
            if sock:
                try:
                    sock.close()
                except OSError:
