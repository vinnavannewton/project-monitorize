...
from monitorize.input_bridge.geometry import Geometry
...
class SwaySupportTest(unittest.TestCase):
    def test_reads_sway_outputs(self):
        ...
        with patch("monitorize.platform.display_controller.subprocess.run", return_value=result):
            self.assertEqual(sway_outputs()[0]["name"], "HEADLESS-1")
    ...
    def test_creates_and_configures_sway_output(self):
        ...
        with (
            patch(
                "monitorize.platform.display_controller.sway_outputs",
                side_effect=[before, after],
            ),
            patch("monitorize.platform.display_controller.subprocess.run", return_value=ok) as run,
        ):
            output, error = prepare_sway_output(2560, 1600, 60)
    ...
    def test_reuses_saved_sway_output(self):
        ...
        ok = Mock(returncode=0, stdout="", stderr="")
        with (
            patch("monitorize.platform.display_controller.sway_outputs", side_effect=[outputs, outputs]),
            patch("monitorize.platform.display_controller.subprocess.run", return_value=ok) as run,
        ):
            output, error = prepare_sway_output(1920, 1080, 60, "HEADLESS-2")
    ...
    def test_disables_sway_output(self):
        with patch(
            ...
            self.assertTrue(disable_sway_output("HEADLESS-3"))
    ...
    def test_maps_sway_input_identifier(self):
        ...
        with (
            ...
            patch("monitorize.input_bridge.geometry.subprocess.run", side_effect=[inputs, mapped]) as run,
        ):
            self.assertTrue(
                Geometry("sway", 2560, 1600).map_sway_devices(["monitorize-touch"])
            )
        self.assertEqual(
