...
from input_bridge.geometry import Geometry
...
class SwaySupportTest(unittest.TestCase):
    def test_reads_sway_outputs(self):
        ...
        with patch("gui.display_controller.subprocess.run", return_value=result):
    ...
    def test_creates_and_configures_sway_output(self):
        ...
        with (
            patch(
                "gui.display_controller.sway_outputs",
                side_effect=[before, after],
            ),
            patch("gui.display_controller.subprocess.run", return_value=ok) as run,
        ):
    ...
    def test_reuses_saved_sway_output(self):
        ...
        ok = Mock(returncode=0, stdout="", stderr="")
        with (
            patch("gui.display_controller.sway_outputs", side_effect=[outputs, outputs]),
            patch("gui.display_controller.subprocess.run", return_value=ok) as run,
        ):
    ...
    def test_disables_sway_output(self):
        with patch(
            "gui.display_controller.subprocess.run",
    ...
    def test_maps_sway_input_identifier(self):
        ...
        with (
            ...
            patch("input_bridge.geometry.subprocess.run", side_effect=[inputs, mapped]) as run,
        ):
            self.assertTrue(
