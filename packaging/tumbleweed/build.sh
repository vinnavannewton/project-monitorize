...
readonly IMAGE="registry.opensuse.org/opensuse/tumbleweed:latest"
...
[[ "$(uname -m)" == "x86_64" ]] || die "The Tumbleweed RPM builder currently supports x86_64 hosts only."
...
tar --sort=name --mtime="@${source_date_epoch}" --owner=0 --group=0 --numeric-owner \
    -C "${tmp_root}/stage" -cf - "${source_name}" | gzip -n -1 > "${source_archive}"
...
PYTHON
        zypper --non-interactive remove monitorize
